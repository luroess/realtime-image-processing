...
Both FSM partitions in `ClickDetector` are Moore-style with respect to output behavior: output controls are derived from current state registers (`s_current_state`, `s_base_current_state`), while button edges only affect next-state transitions. In the RTL, this Moore template is implemented as a clocked state/edge register process (`P_REG_FSM`, #repo_link("rtl/CLICK_DETECTOR/hdl/click_detection.vhd", line: 45, branch: "feat/rollback")) plus combinational next-state and output decode (`P_COMB_FSM` and `P_COMB_BASE_FSM`, #repo_link("rtl/CLICK_DETECTOR/hdl/click_detection.vhd", line: 62, branch: "feat/rollback") and #repo_link("rtl/CLICK_DETECTOR/hdl/click_detection.vhd", line: 104, branch: "feat/rollback")). The Debouncer module and the baseline Moore-FSM process structure used across this control path were implemented by Valentin Bumeder.
...
The control logic is split into two orthogonal Moore partitions. The BTN1 processing FSM in @fig-click-state advances deterministically through `ST_PASS_ALL`, `ST_SOBEL`, and `ST_BLUR_SOBEL`, while `o_pass_blurr_filter`, `o_pass_sobel`, and `o_led[1:0]` are decoded solely from the active state register. Transition conditions depend on debounced rising-edge detection, and output changes occur only after the next registered state is committed.

The BTN2-driven base-image FSM in @fig-click-state-base-current advances through `ST_RGB`, `ST_GRAY`, and `ST_ZEROS` on each debounced BTN2 rising edge. `o_pass_grayscale` toggles between RGB base stream (`ST_RGB`) and grayscale-replicated RGB (`ST_GRAY`), while `o_overlay_zeros` is asserted only in `ST_ZEROS` to disable the base plane and show overlay-only output.
...
The control state machines are verified with cocotb targets registered in #repo_link("testbench/targets.toml", line: 30, branch: "feat/rollback"). For simulation, the debounce time is reduced to keep runtimes short (`G_DEBOUNCE_NS = 100` ns in #repo_link("testbench/targets.toml", line: 36, branch: "feat/rollback"), i.e. 10 cycles at 100 MHz). Existing tesbenches were adapted to

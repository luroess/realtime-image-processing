...
G_BLUR_KERNEL_SIZE = 3
C_STAGE_DELAY_BLOCK = 1024


def _f_stage_delay(line_width: int, kernel_size: int) -> int:
    return (line_width + 1) * ((kernel_size - 1) // 2)


def _f_ceil_div(numerator: int, denominator: int) -> int:
    return (numerator + denominator - 1) // denominator


C_SOBEL_DELAY_CYCLES = _f_stage_delay(G_LINE_WIDTH, G_SOBEL_KERNEL_SIZE)
C_BLUR_DELAY_CYCLES = _f_stage_delay(G_LINE_WIDTH, G_BLUR_KERNEL_SIZE)
C_BLUR_SOBEL_DELAY_CYCLES = C_SOBEL_DELAY_CYCLES + C_BLUR_DELAY_CYCLES

C_SOBEL_CYCLES_CHAINS = _f_ceil_div(C_SOBEL_DELAY_CYCLES, C_STAGE_DELAY_BLOCK)
C_EXTRA_DELAY_CYCLES = C_BLUR_SOBEL_DELAY_CYCLES - C_SOBEL_DELAY_CYCLES
C_EXTRA_CYCLES_CHAINS = _f_ceil_div(C_EXTRA_DELAY_CYCLES, C_STAGE_DELAY_BLOCK)
C_TOTAL_CHAINS = C_SOBEL_CYCLES_CHAINS + C_EXTRA_CYCLES_CHAINS

C_SOBEL_DELAY_EFFECTIVE = C_SOBEL_DELAY_CYCLES + (C_SOBEL_CYCLES_CHAINS - 1)
C_BLUR_SOBEL_DELAY_EFFECTIVE = C_BLUR_SOBEL_DELAY_CYCLES + (C_TOTAL_CHAINS - 1)
...
def _expected_output_for_controls(*, base: Image, mask: Image, overlay_zeros: int) -> Image:
...
async def test_axi_frame_compositor_small_mode_matrix_with_backpressure_and_gray_delays(
    ...
    for overlay_zeros in (0, 1):
        ...
        dut._log.info(
            "small matrix case=%d zeros=%d gray_start_delay=%d need_rgb=%d",
            case_idx,
            overlay_zeros,
            gray_start_delay,
            int(need_rgb),

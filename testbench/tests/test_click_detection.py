...
async def test_click_state_machine(dut) -> None:
    ...
    # BTN2 base FSM in ST_PASS_ALL: RGB -> GRAY -> ZEROS -> RGB.
    ...
    # BTN2 cycling while SOBEL is active: RGB -> GRAY.
    await driver.transition_btn2_state()
    await driver.check_output(0, 1, 0, 10, 30, expected_overlay_zeros=0)

    # SOBEL -> BLUR_SOBEL.
    await driver.transition_btn1_state()
    await driver.check_output(0, 0, 0, 10, 30, expected_overlay_zeros=0)

    # BTN2 while BLUR_SOBEL: GRAY -> ZEROS.
    await driver.transition_btn2_state()
    await driver.check_output(0, 0, 0, 10, 30, expected_overlay_zeros=1)

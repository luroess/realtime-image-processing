...
class FastModeConfig:
    # TODO(mode-config): Keep behavior-driving FAST generics gathered in one place for model/flush parity.
    threshold: int
    n_contiguous: int
    enable_nms: bool
...
def _fast_mode_from_dut(dut: Any) -> FastModeConfig:
    # TODO(mode-readback): Keep FAST generic readback centralized for RTL/model alignment.
...
async def run_fast_case(dut, cfg: FastRunConfig) -> np.ndarray:
    ...
    try:
        ...
        expected = _fast_expected(
            ...
            n_contiguous=mode.n_contiguous,
            enable_nms=mode.enable_nms,
        )

        # AXI_FastFilter has an optional internal 3x3 score-window stage for NMS.
        flush_windows = (
            _warmup_beats(width=width, wndw_size=NMS_WINDOW_SIZE)
            if mode.enable_nms
            else 0
        )
        ...
        require_stall_observation = cfg.expect_stall and (
            mode.enable_nms or ((width * height) >= 256)
        )
        if require_stall_observation:
...
async def test_axi_fast_filter_lenna_end_to_end(dut) -> None:
    ...
    _assert_matches_opencv_interior(
        ...
        threshold=mode.threshold,
        enable_nms=mode.enable_nms,
        min_iou=0.55 if mode.enable_nms else 0.30,
    )

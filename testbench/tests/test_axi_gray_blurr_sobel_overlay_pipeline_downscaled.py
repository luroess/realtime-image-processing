"""Downscaled clone of full RGB->gray->blurr->sobel->overlay pipeline tests."""
...
def _downscaled_full_frame_image() -> Image:
    image_path = TESTBENCH_ROOT / "images" / "lenna_512_512.png"
    full_image = Image.from_png(image_path)
    image = Image(full_image.pixels[::8, ::8, :].copy())
    if image.width != FRAME_WIDTH or image.height != FRAME_HEIGHT:
        raise AssertionError(
            f"Expected downscaled input image size ({FRAME_WIDTH}, {FRAME_HEIGHT}), "

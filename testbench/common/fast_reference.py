...
def _precheck_required(n_contiguous: int) -> int:
    # TODO(precheck-parity): Keep this threshold ladder in lockstep with RTL precheck policy.
    if n_contiguous >= 16:
        return 4
    if n_contiguous >= 12:
        return 3
...
def _fast_scores(
    ...
    *,
    ...
    precheck_required = _precheck_required(n_contiguous)

    for y in range(height):
        for x in range(width):
            center = int(padded[y + 3, x + 3])
            ring = [
                int(padded[y + 3 + dy, x + 3 + dx])
                for (dx, dy) in FAST_RING_OFFSETS
            ]
            ...
            if bright_precheck < precheck_required and dark_precheck < precheck_required:
                continue
...
def _assert_matches_opencv_interior(
    *,
    ...
    opencv_mask = _opencv_fast_mask(gray_plane, threshold=threshold)

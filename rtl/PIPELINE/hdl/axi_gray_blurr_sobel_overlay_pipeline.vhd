...
entity AXI_GrayBlurrSobelOverlayPipeline is
  generic (
...
architecture A_RtlStruct of AXI_GrayBlurrSobelOverlayPipeline is
  ...
  signal s_overlay_rgb_tdata : std_logic_vector((3 * G_PIXEL_WIDTH) - 1 downto 0) := (others => '0');
  signal s_overlay_edge_tdata : std_logic := '0';
  signal s_overlay_pixel : std_logic_vector((3 * G_PIXEL_WIDTH) - 1 downto 0) := (others => '0');
...
begin
  assert G_BLURR_KERNEL_COEFFS'length = (G_BLURR_KERNEL_SIZE * G_BLURR_KERNEL_SIZE * G_BLURR_COEFF_WIDTH)
    report "AXI_GrayBlurrSobelOverlayPipeline: G_BLURR_KERNEL_COEFFS length must equal G_BLURR_KERNEL_SIZE*G_BLURR_KERNEL_SIZE*G_BLURR_COEFF_WIDTH."
    severity failure;
  ...
  s_overlay_rgb_tdata <= s_sobel_tdata & s_sobel_tdata & s_sobel_tdata;
  s_overlay_edge_tdata <= s_sobel_tdata(G_PIXEL_WIDTH - 1);
  ...
  U_EdgeOverlay: entity work.EdgeOverlay
    ...
    port map (
      ...
      i_edge_detected  => s_overlay_edge_tdata,
      i_video_rbg888   => s_overlay_rgb_tdata,
      o_video_rbg888   => s_overlay_pixel
    );

  -- Single-stream AXI timing for pipeline output.
  s_sobel_tready <= '0' when (i_rst_n = '0') else
                    m_axis_video_rbg888_tready;
  m_axis_video_rbg888_tvalid <= '0' when (i_rst_n = '0') else
                                s_sobel_tvalid;

  s_output_idle <= (i_rst_n = '0') or (s_sobel_tvalid = '0');

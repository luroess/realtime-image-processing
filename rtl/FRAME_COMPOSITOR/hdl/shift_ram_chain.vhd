library ieee;
  ...
  -- This module implements BRAM-based shift-register chains to realize the
  -- multi-line delays required for Sobel, FAST, and blur+Sobel overlay
  -- alignment in FrameCompositor. Each requested delay is split into
  -- 1024-beat chunks to fit c_shift_ram_0. A 4-way mux on the output selects
  -- bypass, Sobel tap, blur+Sobel tap, or FAST tap.

entity ShiftRamChain is
  generic (
    -- Delay tap for Sobel-class overlay alignment (in accepted AXI beats).
    G_SOBEL_DELAY      : positive := 1027;
    -- Delay tap for FAST-class overlay alignment (in accepted AXI beats).
    -- FAST delay chain is instantiated only when G_ENABLE_FAST=true.
    G_ENABLE_FAST      : boolean  := true;
    -- Delay tap for FAST-class overlay alignment (in accepted AXI beats).
    G_FAST_DELAY       : positive := 1540;
  ...
  port (
    ...
    -- 00: no delay, 01: sobel delay, 10: blur+sobel delay, 11: fast delay (only when G_ENABLE_FAST=true)
...
architecture A_Rtl of ShiftRamChain is
  ...
  constant C_SEL_FAST       : std_logic_vector(1 downto 0) := "11";
  ...
  -- Keep FAST array dimensions legal even when FAST is disabled.
  function f_fast_chunks(i_fast_enabled : boolean; i_fast_delay : positive; i_block_delay : positive)
    return positive is
  begin
    if i_fast_enabled then
      return f_ceil_div(i_fast_delay, i_block_delay);
    end if;
    return 1;
  end function;

  -- Keep last-delay constant legal in FAST-disabled mode.
  function f_fast_last_chunk_delay(i_fast_enabled : boolean; i_fast_delay : positive; i_fast_chunks : positive; i_block_delay : positive)
    return positive is
  begin
    if i_fast_enabled then
      return f_last_chunk_delay(i_fast_delay, i_fast_chunks, i_block_delay);
    end if;
    return 1;
  end function;
  ...
  constant C_FAST_CHUNKS            : positive := f_fast_chunks(G_ENABLE_FAST, G_FAST_DELAY, C_BLOCK_DELAY);
  constant C_FAST_LAST_CHUNK_DELAY  : positive := f_fast_last_chunk_delay(G_ENABLE_FAST, G_FAST_DELAY, C_FAST_CHUNKS, C_BLOCK_DELAY);
  ...
  signal s_fast_pipe  : t_word_arr_t(0 to C_FAST_CHUNKS) := (others => (others => '0'));
...
begin
  ...
  -- Independent chain realizes FAST-class delay tap (only when enabled).
  G_FAST_CHAIN: if G_ENABLE_FAST generate
    G_FAST_CHAIN_IMPL: for i in 0 to C_FAST_CHUNKS - 1 generate
      constant C_DELAY_I : positive                     := f_chunk_delay(i, C_FAST_CHUNKS, C_BLOCK_DELAY, C_FAST_LAST_CHUNK_DELAY);
      constant C_A_I     : std_logic_vector(9 downto 0) := std_logic_vector(to_unsigned(C_DELAY_I - 1, 10));
    begin
      U_FastChunk: entity work.c_shift_ram_0
        port map (
          A    => C_A_I,
          D    => s_fast_pipe(i),
          CLK  => i_clk,
  ...
  -- FAST-disabled mode aliases selector "11" to Sobel tap and does not
  -- instantiate additional FAST delay RAM stages.
  G_FAST_CHAIN_DISABLED: if not G_ENABLE_FAST generate
  begin
    s_fast_pipe(1) <= s_sobel_pipe(C_SOBEL_CHUNKS);
  end generate;
  ...
  -- Delay-tap mux: bypass, Sobel delay, blur+sobel delay, or FAST delay.
  o_dout <= i_din                        when i_base_delay_stage_sel = C_SEL_NONE else
            ...
            s_fast_pipe(C_FAST_CHUNKS)   when (G_ENABLE_FAST and i_base_delay_stage_sel = C_SEL_FAST) else
            s_sobel_pipe(C_SOBEL_CHUNKS) when ((not G_ENABLE_FAST) and i_base_delay_stage_sel = C_SEL_FAST) else
            i_din;
    ...
    if rising_edge(i_clk) then
      if i_sclr = '1' then
        ...
        if not (
          ...
          (i_base_delay_stage_sel = C_SEL_BLUR_SOBEL) or
          (G_ENABLE_FAST and (i_base_delay_stage_sel = C_SEL_FAST))
        ) then
          if s_invalid_sel_warned = '0' then
            assert false
              report "ShiftRamChain: i_base_delay_stage_sel is illegal; FAST selector 11 requires G_ENABLE_FAST=true."
              severity warning;

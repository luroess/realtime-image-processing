library ieee;
  ...
  -- multi-line delays required for Sobel and blur+Sobel overlay alignment in
  -- FrameCompositor. Each requested delay is split into 1024-beat chunks to
  -- fit c_shift_ram_0.
...
entity ShiftRamChain is
  ...
  port (
    ...
    -- 00: no delay, 01: sobel delay, 10: blur+sobel delay, 11: reserved.
...
architecture A_Rtl of ShiftRamChain is
  ...
  constant C_SEL_RESERVED   : std_logic_vector(1 downto 0) := "11";
...
begin
  ...
  -- Delay-tap mux: bypass, Sobel delay, blur+sobel delay.
  -- Reserved selector "11" aliases to Sobel delay.
  o_dout <= i_din                        when i_base_delay_stage_sel = C_SEL_NONE else
            ...
            s_sobel_pipe(C_SOBEL_CHUNKS) when i_base_delay_stage_sel = C_SEL_RESERVED else
            i_din;
  ...
  begin
    if rising_edge(i_clk) then
      if i_sclr = '1' then
        ...
        if i_base_delay_stage_sel = C_SEL_RESERVED then
          if s_invalid_sel_warned = '0' then
            assert false
              report "ShiftRamChain: i_base_delay_stage_sel=11 is reserved; falling back to Sobel delay tap."
              severity warning;
          ...
          if s_invalid_sel_warned = '0' then
            assert false
              report "ShiftRamChain: i_base_delay_stage_sel has unsupported encoding; falling back to bypass semantics."
              severity warning;
            s_invalid_sel_warned <= '1';
          end if;
        else

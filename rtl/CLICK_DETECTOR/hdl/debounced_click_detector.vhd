...
architecture A_RtlStruct of DebouncedClickDetector is
  ...
  signal s_btn2_debounced : std_logic                    := '0';
  signal s_btn1_vec       : std_logic_vector(3 downto 0) := (others => '0');
  signal s_btn2_vec       : std_logic_vector(3 downto 0) := (others => '0');
...
begin
  ...
  s_btn1_vec <= "000" & i_btn(0);
  s_btn2_vec <= "000" & i_btn(1);

  U_DebouncerBtn1: entity work.Debouncer(A_Rtl) generic map (
  ...
  U_DebouncerBtn2: entity work.Debouncer(A_Rtl) generic map (
    G_CLK_FREQ_HZ => G_CLK_FREQ_HZ,
    G_DEBOUNCE_NS => G_DEBOUNCE_NS
  ) port map (
    i_rst_n         => i_rst_n,
    i_clk           => i_clk,
    i_btn           => s_btn2_vec,
    o_btn_debounced => s_btn2_debounced
  );
  ...
  U_ClickDetector: entity work.ClickDetector(A_Rtl) port map (
    ...
    i_btn2_debounced    => s_btn2_debounced,
  ...
  o_btn_debounced  <= s_btn_debounced;
  o_btn2_debounced <= s_btn2_debounced;
